...
ScreenWidget::ScreenWidget(QWidget *parent) : QWidget(parent)
{
    ...
    menu->addAction(QStringLiteral("截图另存为"), this, SLOT(saveScreenOther()));
...
//    QString fileName = QString("%1/screen_%2.png").arg(qApp->applicationDirPath()).arg(STRDATETIME);
    auto pix = fullScreen->copy(x, y, w, h);
...
//    fullScreen->save(fileName, "png");
    QApplication::clipboard()->setPixmap(*fullScreen);
    signHasImag();
    close();
}
...
void ScreenWidget::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    if (screen->getStatus() == Screen::SELECT)
    {
        ...
        btn->move(x + w - btn->width(),y + h);
        btn->show();
        connect(btn,&QPushButton::clicked,this,&ScreenWidget::saveScreen);
    }
    else if (screen->getStatus() == Screen::MOV)
    {
